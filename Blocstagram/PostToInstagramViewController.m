...
- (void) addCIImageToCollectionView:(CIImage *)CIImage withFilterTitle:(NSString *)filterTitle {
    ...
    if (image) {
        // Decompress image
        //initial code from Bloc caused a crash. changed this setting after reading online and it works.
...
- (void) addFiltersToQueue {
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *ericFilter = [CIFilter filterWithName:@"CIGaussianBlur"];
        
        if (ericFilter) {
            [ericFilter setValue:sourceCIImage forKey:kCIInputImageKey];
            [self addCIImageToCollectionView:ericFilter.outputImage withFilterTitle:NSLocalizedString(@"Eric", @"Eric Filter")];
        }
    }];
    /*
    //Erics Dual filter - doesnt work, got some experience with the crash logs 
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (ericFilter) {
            [ericFilter setValue:sourceCIImage forKey:kCIInputContrastKey];
            
            CIImage *result = ericFilter.outputImage;
            
            if (sepiaFilter) {
                [sepiaFilter setValue:result forKeyPath:kCIInputImageKey];
                [sepiaFilter setValue:result forKeyPath:kCIInputIntensityKey];
                
                result = sepiaFilter.outputImage;
                }
            
            [self addCIImageToCollectionView:result withFilterTitle:NSLocalizedString(@"Dual", @"Dual Filter")];
        }
    }];
    */
